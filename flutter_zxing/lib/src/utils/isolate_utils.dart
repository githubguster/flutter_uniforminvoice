...
class IsolateUtils {
  static const String kDebugName = 'FlutterZxingCppIsolate';
    ...
    await for (final IsolateData? isolateData in port) {
      if (isolateData != null) {
        try {
          ...
          final Uint8List bytes = await ImageConvert.convertImage(image);
          final int width = image.width;
          final int height = image.height;
          final DecodeParams params = isolateData.params;

          if (params.isMultiScan) {
            final List<CodeResult> result = zxingReadBarcodes(
              bytes,
              width: image.width,
              height: image.height,
            ...
            CodeResult result = zxingReadBarcode(bytes,
